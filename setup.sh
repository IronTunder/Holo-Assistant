...
EOF
    ...
    if [ -f "init_db.py" ]; then
        ...
        python init_db.py 2>/dev/null || log "WARNING" "init_db.py potrebbe aver fallito"
    fi
    
    # Popola database
    if [ -f "populate.py" ]; then
        log "INFO" "Popolamento database..."
        python populate.py 2>/dev/null || log "WARNING" "populate.py potrebbe aver fallito"
    fi
    
    # Seed categorie se esiste
    if [ -f "seed_categories.py" ]; then
        log "INFO" "Seeding categorie..."
        python seed_categories.py 2>/dev/null || log "WARNING" "seed_categories.py potrebbe aver fallito"
    fi
...
start_services() {
    ...
    cat > "$ROOT_DIR/start_backend.sh" << 'EOF'
#!/bin/bash
cd "$(dirname "$0")/backend"
source venv/bin/activate
uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
EOF
    chmod +x "$ROOT_DIR/start_backend.sh"
    
    # Script avvio frontend
    cat > "$ROOT_DIR/start_frontend.sh" << 'EOF'
#!/bin/bash
cd "$(dirname "$0")/frontend/my-app"
npm run dev -- --host 0.0.0.0
EOF
    chmod +x "$ROOT_DIR/start_frontend.sh"
    
    # Avvia backend
    log "INFO" "Avvio backend su porta 8000..."
    if command -v gnome-terminal >/dev/null 2>&1; then
        gnome-terminal --title="DITTO Backend" -- bash -c "$ROOT_DIR/start_backend.sh; exec bash" &
    elif command -v xterm >/dev/null 2>&1; then
        xterm -title "DITTO Backend" -e bash -c "$ROOT_DIR/start_backend.sh; exec bash" &
    else
    ...
    if command -v gnome-terminal >/dev/null 2>&1; then
        gnome-terminal --title="DITTO Frontend" -- bash -c "$ROOT_DIR/start_frontend.sh; exec bash" &
    elif command -v xterm >/dev/null 2>&1; then
        xterm -title "DITTO Frontend" -e bash -c "$ROOT_DIR/start_frontend.sh; exec bash" &
    else
